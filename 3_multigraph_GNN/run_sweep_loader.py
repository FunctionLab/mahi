...
def train_model():
    wandb.init(project=project_name, dir="/mnt/home/aaggarwal/ceph/gates_proj/mage_networks/wandb")
    config = wandb.config

    emb_dim_trial = int(config.emb_dim)
    ...
    try:
        model = TransformerMaskedModel(
            ...
            num_neighbors=[config.neighbors] * config.num_layers,
            epochs=epochs,
        ...
        early_stop = EarlyStopping(monitor="val_pr_auc_macro", mode="max", patience=5, verbose=True)
        ...
        trainer = pl.Trainer(
            ...
            strategy="auto",
            logger=WandbLogger(project=project_name, name=f"sweep_{wandb.run.id}"),
        ...
        if "CUDA out of memory" in str(e):
            print(f"[OOM ERROR] Skipping config due to memory issue: {config}")
            torch.cuda.empty_cache()
            os.makedirs("./skipped_configs", exist_ok=True)
            with open(f"./skipped_configs/oom_skipped_configs_{project_name}.log", "a") as f:
...
# run the sweep
def main():
    with open(f"./sweep_ids/sweep_id_{project_name}.txt", "r") as f:
    ...
    wandb.agent(
        ...
        entity="aa8417-princeton-university",
        count=350
    )
